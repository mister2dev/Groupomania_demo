...
exports.likeCount = (req, res) => {
  ...
  const likes = JSON.parse(
    fs.readFileSync(path.join(__dirname, "../database/likes.json"), "utf-8")
  );

  const likesForPost = likes.filter((like) => like.post_id === postId);
  const userHasLiked = likesForPost.some((like) => like.user_id === userId);

  res.status(200).json({
    LikesNumber: likesForPost.length,
    UserLike: userHasLiked,
  });
};
